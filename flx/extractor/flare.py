...
def classify2vector_trans(
    pred_xy: torch.Tensor,
    out_form: str = "claSum",
    trans_num_classes: int = 512,
    eps: float = 1e-6,
) -> torch.Tensor:
    trans_const = 256
    if out_form == "claSum":
...
def classify2vector_rot(
    pred_theta: torch.Tensor,
    out_form: str = "claSum",
    rot_num_classes: int = 180,
    eps: float = 1e-6,
) -> torch.Tensor:
    if out_form == "claSum":
        ...
        with torch.no_grad():
            ...
            if pad_h > 0 or pad_w > 0:
                img_tensor = torch.nn.functional.pad(
                    img_tensor, (0, pad_w, 0, pad_h), mode="replicate"
                )
...
class FLAREFullPipeline:
    ...
    def __init__(
        ...
        # 2 Pose Estimators
        self.pose_voting = FLAREPoseEstimator(
            pose_type="VotingPose", model_path=voting_pose_path, device=device
        )
        self.pose_regression = FLAREPoseEstimator(
            pose_type="RegressionPose", model_path=regression_pose_path, device=device
        )

        # 2 Enhancers
        self.enh_prior = FLAREEnhancer(
            method="PriorEnh", ckpt_dir=priorenh_dir, device=device
        )
        self.enh_unet = FLAREEnhancer(
            method="UNetEnh", model_path=unetenh_path, device=device
        )

        # FDRN (FDD) Descriptor Model
        self.model = FDD(
            ndim_feat=ndim_feat,
            input_norm=input_norm,
            tar_shape=tar_shape,
        ...
        if os.path.exists(desc_model_path):
            load_flare_model(self.model, desc_model_path)
        ...
        with torch.no_grad():
            for batch in tqdm.tqdm(dataloader, desc="Extracting Official FLARE 4-Combination Descriptors"):
                ...
                for idx in range(batch_size):
                    ...
                    four_imgs = torch.cat([comb0, comb1, comb2, comb3], dim=0).to(
                        self.device
                    )

                    # 3. FDRN Descriptor & Mask Extraction for all 4 images
                    outputs = self.model.get_embedding(four_imgs)
...
class FLAREEnhanceFirstPipeline:
    """
    FLARE Pipeline that enhances images before alignment:
    1. Estimate poses using 2 pose estimators (VotingPose & RegressionPose).
    2. Enhance raw image using 2 enhancers (PriorEnh & UNetEnh) -> 2 enhanced images.
    3. Align each enhanced image with each pose -> 4 aligned enhanced images.
    4. Extract dense descriptors & foreground masks using FDD directly.
    """
    def __init__(
        self,
        desc_model_path: str = "../FLARE/model_weights/desc_model.pth.tar",
    ...
    def _enhance_unet_safe(self, img_tensor: torch.Tensor) -> torch.Tensor:
        h, w = img_tensor.shape[-2:]
        pad_h = (16 - (h % 16)) % 16
        pad_w = (16 - (w % 16)) % 16
        if pad_h > 0 or pad_w > 0:
            padded = torch.nn.functional.pad(img_tensor, (0, pad_w, 0, pad_h), mode="replicate")
            enh = self.enh_unet.enhance_tensor(padded)
            return enh[..., :h, :w]
        return self.enh_unet.enhance_tensor(img_tensor)
    ...
    def extract(self, dataset: Dataset) -> FLAREEmbeddingLoader:
        ...
        with torch.no_grad():
            for batch in tqdm.tqdm(dataloader, desc="Extracting FLARE Enhance-First Descriptors"):
                if isinstance(batch, dict) and "img" in batch:
                    raw_imgs = batch["img"].to(self.device)
                else:
                    raw_imgs = batch.to(self.device)
                ...
                for idx in range(batch_size):
                    ...
                    # 1. Pose estimation on input image
                    pose0 = self.pose_voting.predict_pose(img)
                    pose1 = self.pose_regression.predict_pose(img)

                    # 2. Enhancement before alignment
                    enh_prior = self.enh_prior.enhance_tensor(img[None])
                    enh_unet = self._enhance_unet_safe(img[None])

                    # 3. Four Pose Alignments (2 Enhancers x 2 Poses)
                    comb0 = flare_image_transform(
                        enh_prior,
                        tar_shape=self.tar_shape,
                        middle_shape=self.middle_shape,
                        pose_2d=pose0,
                    ).to(self.device)
                    comb1 = flare_image_transform(
                        enh_prior,
                        tar_shape=self.tar_shape,
                        middle_shape=self.middle_shape,
                        pose_2d=pose1,
                    ).to(self.device)
                    comb2 = flare_image_transform(
                        enh_unet,
                        tar_shape=self.tar_shape,
                        middle_shape=self.middle_shape,
                        pose_2d=pose0,
                    ).to(self.device)
                    comb3 = flare_image_transform(
                        enh_unet,
                        tar_shape=self.tar_shape,
                        middle_shape=self.middle_shape,
                        pose_2d=pose1,
                    ...
                    four_imgs = torch.stack([comb0, comb1, comb2, comb3], dim=0).to(self.device)

                    # 4. FDRN Descriptor & Mask Extraction
                    outputs = self.model.get_embedding(four_imgs)
                    feats = outputs["feature"].cpu().numpy()  # [4, 3072]
                    masks = outputs["mask"].cpu().numpy()  # [4, 256]
                ...
                features_all.append(np.stack(batch_features, axis=0))
                masks_all.append(np.stack(batch_masks, axis=0))

        features_array = np.concatenate(features_all, axis=0)
        masks_array = np.concatenate(masks_all, axis=0)

        return FLAREEmbeddingLoader(dataset.ids, features_array, masks_array)


class FLAREAlignOnlyPipeline:
    """
    FLARE Pipeline using pose estimation and affine alignment, skipping enhancement:
    1. Estimate poses using 2 pose estimators (VotingPose & RegressionPose) -> 2 poses.
    2. Warp/align raw image with each pose -> 2 aligned images.
    3. Extract dense descriptors & foreground masks directly using FDRN (FDD) -> 2 representations.
    4. Return FLAREEmbeddingLoader with features and masks for max similarity score matching.
    """
    ...
    def extract(self, dataset: Dataset) -> FLAREEmbeddingLoader:
        ...
        with torch.no_grad():
            for batch in tqdm.tqdm(
                dataloader, desc="Extracting FLARE Align-Only Descriptors"
            ):
                if isinstance(batch, dict) and "img" in batch:
                    raw_imgs = batch["img"].to(self.device)
                else:
                ...
                for idx in range(batch_size):
                    ...
                    two_imgs = torch.stack([aligned0, aligned1], dim=0)

                    # 2. FDRN Descriptor & Mask Extraction
                    outputs = self.model.get_embedding(two_imgs)
                    feats = outputs["feature"].cpu().numpy()  # [2, 3072]
                    masks = outputs["mask"].cpu().numpy()  # [2, 256]
...
# Aliases for standard usage
FLAREPipeline = FLAREFullPipeline
FLAREPreEnhancePipeline = FLAREEnhanceFirstPipeline
FLARENoEnhancePipeline = FLAREAlignOnlyPipeline
FLAREPoseAlignPipeline = FLAREAlignOnlyPipeline
