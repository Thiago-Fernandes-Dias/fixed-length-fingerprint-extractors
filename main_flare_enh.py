...
from flx.extractor.flare import FLAREPipeline
from flx.extractor.enhancement import FLAREEnhancer, get_unetenh_enhancer
...
FOLDERS = [
    ("/mnt/d/Datasets/FVC/FVC2000/Dbs/Db1_a/tif", "../results/FLARE_ENH/fvc_2000_db1_a.csv", A_SUBJECTS),
    ("/mnt/d/Datasets/FVC/FVC2004/Dbs/Db1_a/tif", "../results/FLARE_ENH/fvc_2004_db1_a.csv", A_SUBJECTS),
]
...
def main():
    ...
    pose_model_path = "../FLARE/model_weights/VotingPose.pth"
    enh_model_path = "../FLARE_ENH/pretrained_model/unetenh/unetenh.pth"

    logging.info("Initializing FLARE Pipeline with Fingerprint Enhancement (UNetEnh)...")
    pipeline = FLAREPipeline(
        desc_model_path=desc_model_path,
        pose_type="VotingPose",
        pose_model_path=pose_model_path,
        enhancement_method="UNetEnh",
        enhancement_model_path=enh_model_path,
        device="cuda",
    )
    ...
    for db_path, results_path, subjects in FOLDERS:
        if not os.path.exists(db_path):
            ...
            continue
        logging.info(f"Running FLARE + ENH benchmark on: {db_path}")
