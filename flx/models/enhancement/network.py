...
class VQFPEnhancer_PCNN(nn.Module):
    ...
    def enhance(self, x, w=0):
        ...
        batch_size, _, height, width = z_l.shape
        logits = self.pixelcnn(z_l)
        logits = logits.permute(0, 2, 3, 1).reshape(batch_size, height * width, self.n_embed)
        soft_one_hot = F.softmax(logits, dim=2)
        _, top_index = torch.topk(soft_one_hot, 1, dim=2)
        z_lq = self.quantize.get_codebook_entry(top_index, shape=[batch_size, height, width, self.embed_dim])
